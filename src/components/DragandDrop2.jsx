...
export default function DragandDrop2() {
  ...
  const removeTask = async (tasks) => {
    let newList = Array.from(list);
    const fTasks = tasks.find((task) => task._id === idTask);
    let listRemoved = newList.find(
      (list) => list.tasks.includes(fTasks) === true
    );
    const taskIndex = tasks.indexOf(fTasks, 0);
    const listIndex = list.indexOf(listRemoved, 0);
    await post("/workList/deleteTask", {
      task: fTasks._id,
      idList: listRemoved._id,
    }).catch((error) => console.log(error));
    console.log(taskIndex);
    const restTask = tasks.splice(taskIndex, 1);
    const newColumn = {
      _id: listRemoved._id,
      title: listRemoved.title,
      createDate: listRemoved.createDate,
      creator: listRemoved.creator,
      tasks: taskIndex === 0 ? [] : restTask,
    };
    newList.splice(listIndex, 1, newColumn);
    localStorage.setItem("blist", JSON.stringify(newList));
    updateList(JSON.parse(localStorage.getItem("blist")));
  ...
  return (
    <>
      ...
      {list && (
        <div className="flex justify-center h-full w-max min-w-full relative">
          {(modal3 || modal2) && (
            <div className="w-screen h-full bg-slate-600 absolute bg-opacity-40 z-20"></div>
          )}
          {logged && (
